    ...
    for (const dir of commandFiles) {
      ...
      for (const cmd of commands) {
        ...
        if (
          ...
          console.log(`Kronix | JAI SIYA RAM | + Command Loaded (${command.name})`);
        } else {
          console.log(`Kronix | JAI SIYA RAM | Failed Loading Command :  (${command.name})`);
        }
      }
    }
  } catch (error) {
